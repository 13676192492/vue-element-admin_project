...
<script>
import { getAdminPhoneBillList } from "@/api/phoneBill/index";
  ...
  methods: {
    ...
    orderDetails(num) {
      if (!this.onFetching) {
        ...
        if (this.sipType) this.params.sipType = +this.sipType;
        else if (this.sipType == "") this.params.sipType = null;

        getAdminPhoneBillList(this.params).then(res => {
          if (res.data.success) {
            if (res.data.data) {
              if (res.data.data.items.length > 0) {
                for (let i of res.data.data.items) {
                  ...
                  i.sayTime =
                    updateTime(i.beginTime, 1) +
                    " 至 " +
                    updateTime(i.endTime, 1);
                  if (i.sipType == 0) {
                    i.sipType = "Sip通话";
                  } else {
                    i.sipType = "视频通话";
                ...
                this.params.lastId =
                  res.data.data.items[res.data.data.items.length - 1].id;
              }
            }
          }
          // console.log(num)
