...
<script>
import { getOtherCommunity, bindCommunity } from '@/api/manage/account'
...
export default {
    ...
    data() {
        return {
            ...
            params: {
                page: 1,
                limit: 10,
                search: { userId: undefined, name: undefined }
            },
            doorParams: {
                page: 1,
                limit: 10,
    ...
    methods: {
        ...
        getPlotList() {
            this.loading = true;
            this.params.search.userId = this.row.id;
            let params = {};
            let name = this.params.search.name;
            if(name === undefined || name === '') params = { page: this.params.page, limit: this.params.limit, search: {userId:this.row.id} };
            else params = this.params;
            getOtherCommunity(params).then(response => {
        ...
        connectSure(row){
            this.$confirm(`确定要关联“${ row.name }”小区吗？`, '提示', {
                ...
                bindCommunity(row.id, this.row.id).then(()=>{
                    loading.close();
                    // this.getPlotList();
                    this.$emit("newList");
                    this.$notify({
